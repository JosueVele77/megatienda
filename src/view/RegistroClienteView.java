...
public class RegistroClienteView extends JFrame {
    ...
    public JTextField txtNombre, txtCedula, txtCorreo, txtTelefono, txtDireccion; // <--- Agregado
    public JButton btnGuardar, btnCancelar;

    // Colores
    private final Color COLOR_ACCENT = new Color(59, 130, 246);
    private final Color COLOR_BG_SECONDARY = new Color(40, 40, 45);
    ...
    private void initComponents() {
        ...
        setSize(450, 650); // Aumentamos un poco la altura
        ...
        txtDireccion = crearInput(formPanel, "Dirección Domiciliaria", "home"); // <--- NUEVO CAMPO
    ...
    public void marcarCampoError(JTextField campo, boolean error) {
        if (error) campo.putClientProperty(FlatClientProperties.OUTLINE, "error");
        else campo.putClientProperty(FlatClientProperties.OUTLINE, null);
        campo.repaint();
    }

    // Clase Iconos actualizada con "home"
    private static class SimpleIcon implements Icon {
        ...
        @Override public void paintIcon(Component c, Graphics g, int x, int y) {
            ...
            if(type.equals("user")) { g2.drawArc(cx-3, cy-5, 6, 6, 0, 360); g2.drawArc(cx-5, cy+2, 10, 8, 0, 180); }
                ...
                // Dibujo simple de casa
                int[] xP = {cx-5, cx, cx+5};
                int[] yP = {cy-2, cy-7, cy-2};
                g2.drawPolyline(xP, yP, 3); // Techo
                g2.drawRect(cx-4, cy-2, 8, 7); // Base
            }
            g2.dispose();
        }
